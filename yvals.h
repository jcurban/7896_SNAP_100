...
#if __STM8ABI_PORTABILITY_INTERNAL_LEVEL && !_DLIB_SUPPORT_FOR_STM8ABI
  #error "__STM8ABI_PORTABILITY_LEVEL != 0 needs a library built with _DLIB_SUPPORT_FOR_STM8ABI turned on"
#endif
...
_END_EXTERN_C
...
#if _DLIB_SUPPORT_FOR_STM8ABI && !_DLIB_MBSTATET_USES_UNSIGNED_LONG
  typedef struct
  ...
  #endif
#else /* _DLIB_SUPPORT_FOR_STM8ABI */
  typedef struct
  {       /* state of a multibyte translation */
...
#endif /* _DLIB_SUPPORT_FOR_STM8ABI */
...
#include <DLib_Threads.h>
  ...
  _C_STD_BEGIN
  _EXTERN_C
  __ATTRIBUTES void __iar_Locksyslock(unsigned int);
  __ATTRIBUTES void __iar_Unlocksyslock(unsigned int);
  _END_EXTERN_C
  _C_STD_END
 ...
 */
  ...
  public:
  #if _MULTI_THREAD && !(!_DLIB_FILE_DESCRIPTOR && _DLIB_THREAD_SUPPORT == 3)
    __iar_Mutex();
    ~__iar_Mutex();
    void _Lock();
    void _Unlock();
